...
if r.status_code != 403:
    ...
    print(a)
    print(b)
    df2 = pd.DataFrame(a,columns =['First_name', 'Last_name', 'Emailaddress', 'useractiveflag', 'userdeleteddate', 'senttimestamp', 'Time_stamp', 'eventtype', 'campaignname', 'autoenrollment', 'campaignstartdate', 'campaignenddate', 'campaigntype', 'campaignstatus', 'templatename', 'templatesubject', 'assessmentisarchived', 'usertags', 'sso_id'])
    print(df2)
    result = pd.concat([df1, df2], axis=1, join="outer")
    # result1= result['First_name'],['Last_name']
    result1= result.loc[:,'First_name': 'Time_stamp']  # (isto:) method used for taking the some columns0
    print(result1)
    print(result)
    # send_sns_success()
